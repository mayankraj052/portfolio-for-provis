...
import { Mail, Send, Github, Linkedin, Instagram, Twitter, ExternalLink } from "lucide-react";
    ...
    <section id="contact" className="relative py-20 md:py-32 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <BadgeText icon={Send} text="Let's Connect" />
        ...
        <div className="relative lg:flex lg:gap-12 space-y-12 lg:space-y-0">
          {/* Contact Form */}
          <div className="lg:w-1/2 w-full">
            <div className="group relative">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-violet-600 via-fuchsia-600 to-indigo-600 rounded-2xl opacity-0 group-hover:opacity-100 blur transition duration-500" />
              <div className="relative bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-8">
                <div className="absolute inset-0 bg-gradient-to-br from-violet-500/5 to-indigo-500/5 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                <div className="space-y-6 relative">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white rounded-lg font-medium transition-all duration-300 group-hover:shadow-lg group-hover:shadow-violet-500/30"
                  >
          ...
          <div className="lg:w-1/2 w-full flex flex-col gap-6 mt-5 lg:mt-0">
            {/* Contact Info */}
            {contactInfo.map((info, idx) => {
              const Icon = info.icon;
              return (
                <a key={idx} href={info.href} className="group relative">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-xl opacity-0 group-hover:opacity-100 blur transition duration-300" />
              ...
              {socialLinks.map((social, idx) => {
                ...
                return (
                  <a key={idx} href={social.url} target="_blank" rel="noopener noreferrer" className="group relative">
                    <div className={`absolute -inset-0.5 bg-gradient-to-r ${social.color} rounded-xl opacity-0 group-hover:opacity-100 blur transition duration-300`} />
